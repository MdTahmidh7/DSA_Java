...
public class RBS_DuplicateValue {
    ...
    public static int findPivotWithDuplicate(int [] arr){
        ...
        while (start <= end){
            ...
            //Here mid, start and end become the same element
            // Here we can skip the same element
            else if(arr[mid]== arr[start] && arr[mid] == arr[end]){
                // skip from both side. But
                //check if start  or end is the pivot
                if(arr[start]>arr[start+1]){
                    return start;
                }else{
                    start++;
                }if(arr[end] < arr[arr[end-1]]){
                    return end-1;
                }else{
                    end--;
                }
